...
<h6>При помощи этой вкладки Вы можете переключать макеты страницы товара. Она отобразится в админ панели после того как Вы выполните простые действия, описанные ниже.</h6>
...
<ul class="index-list">
    ...
    <li>В папке шаблона найдите файл 'product.tpl', скопируйте его в созданную папку 'product_pages' и переименуйте на 'default.tpl'. Для того чтобы создать несколько макетов -  дублируйте этот файл, переименуйте его (e.g. 'layout_1.tpl', 'layout_2.tpl') и измените структуру в соответствии с вашими потребностями.</li>
  ...
  "default": {
    ...
    "default": 1
        ...
        <ul class="marked-list">
            ...
            <li>"preview": <strong>"default.jpg"</strong> - картинка предварительного просмотра макета страницы товара в настройках модуля; изображение должно быть размещено в папке 'product_pages'.
            </li>
            <li><p>"default": <strong>1</strong> - задайте значение '1', если макет должен использоваться как макет по умолчанию</p><br>
                <p class="alert small alert-warning">Один из макетов должен содержать значение '1', для того чтобы использоваться как макет по умолчанию.</p>
            </li>
        </ul>
        <p>Этот код нужно дублировать для каждого созданного макета. Он должен содержать соответствующие значения. Например:</p>
        <pre class="codebox">
{
  "default": {
    "name": "Default",
...
{/if}</pre>
    ...
    <li>Для того чтобы добавить/изменить стили любого макета страницы товара, перейдите в папку your_theme_folder/css/ и добавьте новую папку 'product_pages'. В этой папке создайте файл с названием макета и расширением .css (например, 'default.css', 'layout_1.css', 'layout_2.css'). Эти файлы должны содержать стили соответствующего макета страницы товара.</li>
    <li>Для того чтобы добавить скрипты к любому макету страницы товара, перейдите в папку your_theme_folder/js/ и добавьте новую папку 'product_pages'. В этой папке создайте файл с названием макета и расширением .js (например, 'default.js', 'layout_1.js', 'layout_2.js'). Эти файлы должны содержать скрипты соответствующего макета страницы товара.</li>
</ul>
...
<p>В этой вкладке Вы можете переключить макет страницы товара. Изображение макета с серой границей задано как стандартное.</p>
